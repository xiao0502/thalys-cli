...
export const TOKEN_KEY = 'Access-Token-Spd';
 ...
 * @returns {Array}
 */
export const backendMenusToRouters = (menus, access) => {
    let routers = [];
    forEach(menus, menu => {
        // 将后端数据转换成路由数据
        let route = backendMenuToRoute(menu, access);
        // 如果后端数据有下级，则递归处理下级
 ...
 * @returns {Object}
 */
const backendMenuToRoute = (menu, access) => {
    ...
    switch (route.level) {
        ...
        case 2:
            ...
            break;
        case 3:
            obj.component = () => import(`@/view/${route.name}/index.vue`);
            break;
    }
    return obj;
};
